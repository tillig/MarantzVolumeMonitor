...
After setup, tap the small gear icon in the top-right corner of the Home Screen to reopen Settings. Settings provides a read-only `Current Settings` overview, Wi-Fi setup, receiver setup, and touch calibration. Use the Settings OK button to return to the Home Screen.
...
The Home Screen currently shows volume on the normalized `0.0` to `100.0` scale used by the firmware (`receiver dB + 80`) with one decimal place rather than showing the raw negative dB value from the receiver.
